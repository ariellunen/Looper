...
const App = () => {
	...
	//Update stop state -> setting time to 0 - the audio will start from the beginning
	const stop = () => {
		audio.forEach((track) => (track.audioRef.currentTime = 0));
	};

	//Update Loop state after pressing the button
	const loop = () => {
		setLoop(!isLoop)
	};

	//Update play button after pressing the button
		...
		<React.Fragment >
			...
			<Container component="main" maxWidth="sm" sx={{ mb: 4 }}>
				<Paper sx={{ my: { xs: 3, md: 6 }, p: { xs: 2, md: 3 } }}>
					<Cursor currTime={currTime} setCurrTime={setCurrTime} handleTimer={handleTimer} />
